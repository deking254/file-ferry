...
document.addEventListener("DOMContentLoaded", function () {
    ...
    typeElement.value = urlParams.get('type');
    typeElement.disabled = true;
    // dataField.value = urlParams.get('data');
    if (urlParams.get('isPublic') === 'true') {
        isPublic.selectedIndex = 0;
    } else {
        isPublic.selectedIndex = 1;
    }
    const folders = get_folders(token).then(data => {
        ...
        parent.value = urlParams.get('parentId');
    }).catch((error) => {
        console.error('Error:', error);
    });;
    //parent.value = urlParams.get('parentId');
    if (urlParams.get('type') === 'folder') {
        dataField.disabled = true;
    }
    //The code below is commented out because no changes should be made to the type of data structure
